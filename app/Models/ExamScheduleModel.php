...
class ExamScheduleModel extends Model
{
    ...
    {
        return ExamScheduleModel::select('exam_schedule.*', 'subject.name as subject_name', 'class.name as class_name', 
                            'exam.name as exam_name')
                                ->join('assign_class_teacher', 'assign_class_teacher.class_id', '=', 'exam_schedule.clsss_id')
                                ->join('subject', 'subject.id', '=','exam_schedule.subject_id')
                                ->join('class', 'class.id', '=', 'class.clsss_id')
                                ->join('exam', 'exam.id', '=', 'exam_schedule.exam_id_id')
                                ->where('assign_class_teacher.teacher_id', '=', $teacher_id)
                                ->get();
